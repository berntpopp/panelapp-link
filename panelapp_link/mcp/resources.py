...
PANELAPP_USAGE_NOTES = (
    ...
    "extras. Paged tools (search_panels, get_panel_genes) page "
    "via an opaque truncated.next_cursor surfaced as _meta.next_commands[0]; "
    "follow it to page through results. get_gene_panels is NOT paged -- it returns "
    "every panel the gene appears on in one call. Compare gene sets across panels with "
...
PANELAPP_REFERENCE_NOTES = (
    ...
    "both (queries and merges across regions; get_panel and get_panel_genes "
    "require a single concrete region, because panel ids are per-region). "
    ...
    "if malformed. get_gene_panels takes no cursor: it returns every panel for the "
    "gene in one call."
)
