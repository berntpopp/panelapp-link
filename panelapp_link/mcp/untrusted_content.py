...
def sanitize_message(text: str) -> str:
    ...
    string so a hostile upstream (or a caller-influenced 4xx/5xx body) can never
    smuggle control, zero-width, bidirectional, or NUL code points into an error
    frame. Reuses the same ``FORBIDDEN_CODEPOINTS`` set the untrusted-text fence
    removes. Caller-visible messages are server-authored guidance data, never
    instructions; upstream response bodies are additionally kept out of them at the
    source (severed at the API client), because stripping code points does NOT
    remove injection prose.
    """
